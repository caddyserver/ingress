...
func NewStore(
	...
	if err := ingressCache.AddIndexers(map[string]cache.IndexFunc{
		...
		isControlledIngressIndex: func(obj interface{}) ([]string, error) {
	...
	if err := endpointSliceCache.AddIndexers(map[string]cache.IndexFunc{
		...
		endpointSlicesByServiceNameIndex: func(obj interface{}) ([]string, error) {
...
func (store *Store) Ingresses() []*apinetworking.Ingress {
	// Note: errors only when the index does not exist.
	list, _ := store.ingressCache.ByIndex(isControlledIngressIndex, yesIndexValue)
...
func (store *Store) EndpointSlicesByService(serviceName string) []*apidiscovery.EndpointSlice {
	// Note: errors only when the index does not exist.
	list, _ := store.endpointSliceCache.ByIndex(endpointSlicesByServiceNameIndex, serviceName)
...
// The serviceName must be specified in full as `<namespace>/<name>`
func (store *Store) Service(serviceName string) *apicore.Service {
	if service, exists, _ := store.serviceCache.GetByKey(serviceName); exists {
...
func (store *Store) SecretMeta(secretName string) *apicore.Secret {
	if secret, exists, _ := store.secretCache.GetByKey(secretName); exists {
