...
func (r *QueuedEventHandlers[T]) OnAdd(obj interface{}, isInInitialList bool) {
	if obj, ok := obj.(*T); ok {
...
func (r *QueuedEventHandlers[T]) OnUpdate(oldObj, newObj interface{}) {
	oldObjT, _ := oldObj.(*T)
	newObjT, _ := newObj.(*T)
...
func (r *QueuedEventHandlers[T]) OnDelete(obj interface{}) {
	if r.DeleteFunc != nil {
		if obj, ok := obj.(*T); ok {
			if r.FilterFunc == nil || r.FilterFunc(obj) {
